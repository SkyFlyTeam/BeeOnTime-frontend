...
import SolicitacaoFolgaInterface from "@/interfaces/SolicitacaoFolga"; // Importando a interface de solicitação de folga
...
const SolicitarFolgaModal: React.FC<SolicitarFolgaModalProps> = ({
  ...
  try {
    ...
    const response = await bancoHorasServices.getBancoHorasSaldoAtual(usuarioLogadoCod, today);
    console.log("Resposta da API:", response);

    if (response instanceof ApiException) {
      toast.error("Erro ao buscar banco de horas: " + response.message);
    } else {
      setSolicitacao((prev) => ({
        ...prev,
        bancoDeHoras: response.bancoHorasSaldoAtual ?? 0,  // ajustar saldoAtual para o nome correto da propriedade da API
      }));
    }
  } catch (error) {
 ...
 // Use o estado de usuarioCod para garantir que a requisição seja feita
